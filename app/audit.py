...
ACTION_LABELS = {"create": "Eklendi", "update": "Düzenlendi", "delete": "Silindi", "replace": "Dosya değiştirildi", "crop": "Görsel kırpıldı", "reorder": "Sıralandı"}
FIELD_LABELS = {
    ...
    for action, objects in (("create", session.new), ("update", session.dirty), ("delete", session.deleted)):
        for obj in list(objects):
            ...
            session.add(AuditLog(
                ...
                label=label[:300], changes=json.dumps(changes, ensure_ascii=False),
            ))


def add_event(session: AsyncSession, action: str, entity: str, label: str, entity_id: int | None = None, changes: dict | None = None) -> None:
    actor = session.info.get("audit_actor")
    if actor:
        session.add(AuditLog(actor=actor, action=action, entity=entity, entity_id=entity_id, label=label[:300], changes=json.dumps(changes or {}, ensure_ascii=False)))
