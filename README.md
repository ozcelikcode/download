...
The default database is `download.db`. Files named `download.db-wal` and `download.db-shm` are SQLite working files, not backup copies. Do not delete them while the application is running.
